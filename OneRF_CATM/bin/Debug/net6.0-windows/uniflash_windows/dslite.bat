...
set GENERATED_COMMAND=-c user_files/configs/cc1312r1f3.ccxml -l user_files/settings/generated.ufsettings -s VerifyAfterProgramLoad="No verification" -e -f -v "user_files/images/apnenel.bin,0x0" 
set ADDITIONALS= -a "BlankCheck" -a "ReadPriIeee"
...
if "%1" EQU "" (
	...
	CMD /S /C "%EXECUTABLE% !MODE! !GENERATED_COMMAND! !ADDITIONALS! > output.txt 2>&1"
	exit /b !errorlevel!
)
...
CMD /S /C "%EXECUTABLE% !MODE! !USEROPTIONS! !ADDITIONALS! > output.txt"
exit /b !errorlevel!